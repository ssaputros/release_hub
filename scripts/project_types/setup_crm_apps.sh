...
if [ "$BRANCH_EXISTS" == "true" ]; then
    ...
    if command -v git_checkout_or_use_worktree >/dev/null 2>&1; then
        if ! git_checkout_or_use_worktree "$ID"; then
            echo "  ❌ Harap commit/stash perubahan Anda atau gunakan --worktree-path ke worktree branch '$ID'."
            exit 1
        fi
        LOCATION=$(pwd)
        echo "  📍 Lokasi aktif: $LOCATION"
        git_pull_current_branch "$ID"
    else
    ...
    if command -v git_create_branch_from_base >/dev/null 2>&1; then
        if ! git_create_branch_from_base "$ID" "$STABLE_BRANCH"; then
            exit 1
        fi
    else
