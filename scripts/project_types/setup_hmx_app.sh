...
OLD_IOS_BUNDLE=$(grep "PRODUCT_BUNDLE_IDENTIFIER" "ios/App/App.xcodeproj/project.pbxproj" 2>/dev/null | sed 's/.*PRODUCT_BUNDLE_IDENTIFIER = \(.*\);.*/\1/' | head -n 1 | tr -d ' ' | tr -d '"')
...
if [ -n "$OLD_IOS_BUNDLE" ]; then
    if [ "$OLD_IOS_BUNDLE" != "$APP_PACKAGE_NAME" ]; then
        ...
        echo "     - $OLD_IOS_BUNDLE"
        echo "     + $APP_PACKAGE_NAME"
        sed -i '' "s/$OLD_IOS_BUNDLE/$APP_PACKAGE_NAME/g" "ios/App/App.xcodeproj/project.pbxproj" 2>/dev/null
    else
        echo "  ✅ iOS Bundle OK:"
        echo "     $APP_PACKAGE_NAME"
    fi
...
if [ -f "ios/App/App/Info.plist" ]; then
    perl -0777 -pi -e "s/(<key>CFBundleDisplayName<\/key>\s*<string>)[^<]*(<\/string>)/\1$APP_NAME\2/g" "ios/App/App/Info.plist" 2>/dev/null
    perl -0777 -pi -e "s/(<key>CFBundleName<\/key>\s*<string>)[^<]*(<\/string>)/\1$APP_NAME\2/g" "ios/App/App/Info.plist" 2>/dev/null
fi
